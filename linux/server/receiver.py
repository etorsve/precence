...
#    except:
#        sys.exit("Connection to MySQL failed: "+str(pymysql.err))
...
def presence_update(db, payload):
    cursor = db.cursor()
    # See if sensor already exists in sensors table, if not insert it, if so update it with latest information.
    #deviceQuery = "EXISTS(SELECT * FROM presence WHERE date = '%s')"%(payload['date'])
    #cursor.execute("SELECT "+deviceQuery)
    #data = cursor.fetchone()
    #if(data[deviceQuery] >= 1):
    #    updateRequest = "UPDATE sensors SET transmission_count = %i, battery_level = %.2f, rssi = %i, last_hear$
    #    cursor.execute(updateRequest)
    #    db.commit()
    #else:
    insertRequest = "INSERT INTO presence(date, time, tag) VALUES('%s','%s','%s')" % (payload['date'], payload['time'], payload['tag'])
    cursor.execute(insertRequest)
    db.commit()
...
def on_message(client, userdata, msg):
    ...
    print("payload.date='"+payload['date']+"'")
    print("payload.time='"+payload['time']+"'")
    print("payload.tag='"+payload['tag']+"'")
